...
def test_real_rostam_records_are_clean():
    """Every record captured on the cluster: no unknowns, no errors."""
    for path in sorted(ROSTAM.glob("*.json")):
        record = Fingerprint.from_json(json.loads(path.read_text()))
        findings = doctor.diagnose(record, local=False)
        assert not [f for f in findings if f.level in (doctor.BLIND, doctor.SUSPECT)], (path.name, findings)
